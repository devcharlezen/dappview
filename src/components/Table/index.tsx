...
function Table({ columns = [], data = [], align, gap, pinnedRow, ...props }: ITableProps) {
	...
	const [initialData, newData] = React.useState(sortedData.slice(pagesVisted, pagesVisted + usersPerPage));
	const pageCount = Math.ceil(sortedData.length / usersPerPage);
	const changePage = ({selected}) =>{
		setPageNumber(selected)
		newData(sortedData.slice(pagesVisted, pagesVisted + usersPerPage))
	}
	const displayData = <TableWrapper>
				<thead>
					<RowWrapper>
						{columns.map((col) => {
				...
				<tbody>
					...
					{initialData.map((item, index) => (
						<Row key={uuid()} item={item} index={index + pagesVisted} columns={columns} />
					))}
				</tbody>
			</TableWrapper>
